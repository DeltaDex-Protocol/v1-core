...
describe("Multiple User Replication", () => {
  ...
  it("Should set addresses inside periphery", async () => {
    ...
    let storageAddr2 = await bscoptionmaker
      .connect(accounts[0])
      .getStorageAddr();
    console.log("storage addr: ", storageAddr2);

    await bscoptionmaker.connect(accounts[0]).setCoreAddr(optionmaker.address);

    let coreAddr2 = await bscoptionmaker.connect(accounts[0]).getCoreAddr();
  ...
  it("Position 3: JDM Call replication: ", async () => {
    // input to JDM start replication
    let tokenA_balance = "5000";
    let amount = "1";
    let fee = "400";
  ...
  it("Position 4: JDM Put replication: ", async () => {
    // input to JDM start replication
