...
contract OptionStorage is PairStorage {
    ...
    function addInitialAmounts(address pair, address user, uint ID, uint tokenA_balance, uint tokenB_balance) external onlyTrusted returns (bool) {
    ...
    function BS_addFee(address pair, address positionOwner, uint ID, uint feeAmount) external onlyTrusted returns (bool) {
    ...
    function BS_Options_updateTimeStamp(address pair, address user, uint ID, uint lastHedgeTimeStamp) external onlyTrusted {
        BS_Options[pair][user][ID].lastHedgeTimeStamp = lastHedgeTimeStamp;
    }

    function BS_Options_updateHedgeFee(address pair, address user, uint ID, uint subFee) external onlyTrusted {
        BS_Options[pair][user][ID].fees -= subFee;
    }

    function BS_Options_subA_addB(address pair, address user, uint ID, uint subA, uint addB) external onlyTrusted {
        BS_Options[pair][user][ID].tokenA_balance -= subA;
        BS_Options[pair][user][ID].tokenB_balance += addB;
    }

    function BS_Options_subB_addA(address pair, address user, uint ID, uint subB, uint addA) external onlyTrusted {
    ...
    function BS_allPositionParams(address pair, address user, uint ID) external view returns (BS.BS_params memory) {
        return BS_Options[pair][user][ID];
    }

    function getInitialBalance(address pair, address user, uint ID) external view returns (uint tokenA_balance, uint tokenB_balance) {
    ...
    function BS_getDeltaParams(address pair, address user, uint ID) external view returns (int K, int T, int r, int sigma, bool isCall) {
    ...
    function BS_PositionParams(address pair, address user, uint ID) external view returns (uint amount, uint expiry, uint fees, uint perDay, uint hedgeFee, uint lastHedgeTimeStamp) {
    ...
    function BS_tokenAddr(address pair, address user, uint ID) external view returns (address tokenA, address tokenB) {
    ...
    function BS_Options_contractAmount(address pair, address user, uint ID) external view returns (uint amount) {
        amount = BS_Options[pair][user][ID].amount;
        return amount;
    }
    ...
    function BS_getWithdrawParams(address pair, address user, uint ID) external view returns (address tokenA, address tokenB, uint tokenA_balance, uint tokenB_balance, uint feeBalance) {
        tokenA = BS_Options[pair][user][ID].tokenA;
        tokenB = BS_Options[pair][user][ID].tokenB;
